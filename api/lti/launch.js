...
const LTI_PRIVATE_KEY = process.env.LTI_PRIVATE_KEY?.replace(/\\n/g, "\n");
const LTI_KEY_ID = process.env.LTI_KEY_ID || "pulmolearn-lti-key-1";
...
function getCanvasJwks() {
  ...
  if (!canvasJwks) {
    canvasJwks = createRemoteJWKSet(new URL(CANVAS_JWKS_URL));
  }
...
export default async function handler(req, res) {
  try {
    ...
    // ────────────────────────────────────────────────────────────────
    // INSTRUCTOR: DEEP LINKING / ASSIGNMENT PICKER
    // ────────────────────────────────────────────────────────────────
...
<body style="font-family:Arial,sans-serif;padding:24px;background:#f4fafc;color:#0b1f33;">
  ...
  <div style="background:white;border:1px solid #d7e6ef;border-radius:18px;padding:20px;margin-bottom:16px;">
    ...
    //
    // Always hand the Canvas identity to login.html through a signed,
    // short-lived token. If the learner already has a PulmoLearn
    // session, login.html immediately calls /api/lti/link and continues.
    // If not, the learner signs in first.
    // ────────────────────────────────────────────────────────────────
